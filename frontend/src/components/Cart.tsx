    ...
    <>
      ...
      <div className=" w-[100vw] h-full flex p-[2vw]">
        ...
        <div className=" w-2/6 h-[20vw] bg-white justify-evenly flex flex-col items-center rounded-4xl">
          <span data-testid="cost">Суммарная стоимость заказа: {totalSum.toLocaleString('ru-RU')} ₽</span>
          <span className=" flex items-center w-[40%] justify-between">Количество товаров: <u className=" text-2xl">{productsID.length}</u></span>
          <button data-testid="createOrder" onClick={() => productsID.length && dispatch(createOrder(orderInfo))} className={`  w-[65%] h-[20%] rounded-4xl  text-2xl ${productsID.length ? "cursor-pointer bg-amber-100" : "cursor-not-allowed bg-gray-200"}`}>Создать Заказ</button>
        </div>
      </div>
    </>
  )
